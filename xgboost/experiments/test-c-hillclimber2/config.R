#' HILLCLIMBER2
#' changes from test-b-hillclimber:
#' - use more training data
#'   - I think earlier ones don't have enough
#' - include lambda and alpha in parameter search
#'   - _technically_ should leave these as zero because our inputs are not scaled
#'     but I'm deciding to leave it up to the hillclimber to find out if they are useful or not
#' - change proportions for integer hyperparams directions so they follow more of a normal dist

# initial params taken from param_id 35 of test-b
initial_params =
  list(
    nthread = 6,
    eta = 0.142781300483726,
    gamma = 0.885713193401288,
    max_depth = 8,
    min_child_weight = 1.06332970202177,
    max_delta_step = 0,
    subsample = 0.972790493855039,
    sampling_method = "uniform",
    colsample_bytree = 1,
    colsample_bylevel = 1,
    colsample_bynode = 0.990455459894892,
    lambda = 0,
    alpha = 0,
    tree_method = "hist",
    num_parallel_tree = 6
...
with_shifted_seed = function(seeds, ex) {
  old.seed = .Random.seed
  base_seed = 205757
  shift_seed = function(s) runif(1, min = 0, max = .Machine$integer.max - 10485760) %>% floor() %>% `+`(s) %>% set.seed()

  set.seed(base_seed)
  sapply(seeds, shift_seed)
...
new_hc_direction = function(round_id, run_id, param_id) {
  ...
  hc_direction =
    list(
      ...
      d_max_depth         = sample(c(-1, 0, 1), 1, prob = c(1, 3, 1)),
      ...
      d_lambda            = rnorm(1, sd = 0.01),
      d_alpha             = rnorm(1, sd = 0.01),
      d_num_parallel_tree   = sample(c(-1, 0, 1), 1, prob = c(1, 3, 1))
    )
...
hc_step = function(params, hc_direction) {
  ...
  newparams =
    list(
      ...
      lambda            = clamp(params[["lambda"]]            + hc_direction[["d_lambda"]]           , 0, NULL),
      alpha             = clamp(params[["alpha"]]             + hc_direction[["d_alpha"]]            , 0, NULL),
